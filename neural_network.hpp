...
public:
    ...
    bool backPropagate(std::vector<float> targetOutput)
    {
        ...
        Matrix temp = _valueMatrices.back();
        temp = temp.negetive();
        ...
        for (int32_t i = _weightMatrices.size() - 1; i >= 0; i--)
        {
            ...
            Matrix temp = _weightMatrices[i];
            temp = temp.transpose();
            Matrix<float> prevErrors = errors.multiply(temp);
...
{
template<class Archive>
    void serialize(Archive & ar, SimpleNeuralNetwork &n, const unsigned version)
    {
        ar & n;
    }
}
